#!/usr/bin/env python
"""patch_manifest_131.py -- factory 1.30.0 -> 1.31.0

WHAT
  Re-pins four tools: deli_counter 0.90.0, level_factory 0.47.0,
  pixelcoat 0.15.0, zoo 0.44.0. Corrects the pixelcoat note (it claimed
  there is no root VERSION file; 0.13.0 added one). Rewrites the
  description's leading block and demotes the previous one to PRIOR
  CERTIFICATION.
...
TAGS ARE RESOLVED, NOT ASSUMED
  ...
  * REQUIRE -- every tag this patch WRITES must already exist inside that
    tool's OWN repository. Missing one refuses the whole write. This is
...
USAGE   (run from anywhere; paths are absolute)
  python patch_manifest_131.py --check
  python patch_manifest_131.py
  python patch_manifest_131.py --selftest
  python patch_manifest_131.py --revert
"""
import argparse
import hashlib
import io
...
def _selftest(fails):
    ...
    # 1. the pins landed
    if man["factory_version"] != "1.31.0":
        fails.append("factory_version is %r" % man["factory_version"])
